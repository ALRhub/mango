    ...
    for dirpath, dirnames, filenames in os.walk(base_path):
        ...
        if obj_type in ["gth", "pred", "collider"]:
            ...
            if obj_type == "gth":
                ...
                # Enable 'Render Lines As Tubes'
                mesh_display.RenderLinesAsTubes = 1
                # Set the line width
                mesh_display.LineWidth = 5
            elif obj_type == "pred":
...
def load_visualization(iter_path, traj_idx, hide=False, process_only=False):
    ...
    # load predicted pos
    file_name = os.path.join(iter_path, f"Traj_{traj_idx}.npz")
    all_def_positions = np.load(file_name)
    all_faces = np.load(os.path.join(vis_path, "faces_data", "faces.npz"), allow_pickle=True)
    load_mesh(all_def_positions, all_faces, gth=False, base_folder=base_folder)
    # load gth pos
    file_name = os.path.join(vis_path, "ground_truth_data", f"Traj_{traj_idx}.npz")
    all_def_positions = np.load(file_name)
    draw_floor = load_mesh(all_def_positions, all_faces, gth=True, base_folder=base_folder)
    # load collider
    file_name = os.path.join(vis_path, "collider_data", f"Traj_{traj_idx}.npz")
    # only load collider if file exists
    if os.path.exists(file_name):
        all_collider_positions = np.load(file_name)
        load_collider(all_collider_positions, all_faces, base_folder=base_folder)

    if not process_only:
        # now all mesh files are created, load them in paraview
        show_in_paraview(base_folder=base_folder, color=color, hide=hide)
    return draw_floor
...
# traj_idx = "0"
if traj_idx == " " or traj_idx.lower() == "all":
    first_traj = True
    for file in sorted(os.listdir(iter_path)):
        if file.startswith("Traj_") and file.endswith(".npz"):
            traj_idx = file.split("_")[1].split(".")[0]
            draw_floor = load_visualization(iter_path, traj_idx, hide=not first_traj, process_only=process_only)
            first_traj = False
else:
